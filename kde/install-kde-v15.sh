...
echo "Instalando Waterfox..."

sudo -u $SUDO_USER flatpak install -y flathub net.waterfox.waterfox
...
sudo -u $SUDO_USER flatpak install -y flathub com.rtosta.zapzap
...
if [[ "$INSTALL_SYSUPDATE" =~ ^[sS]$ ]]; then
  ...
  wget -O /usr/local/bin/system-update \
    https://raw.githubusercontent.com/acierto-incomodo/StormStore/main/kde/system-update.sh
